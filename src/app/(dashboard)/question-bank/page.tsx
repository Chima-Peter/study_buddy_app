...
import { listQuestionBanks } from "@/lib/api/question-bank";
...
export default function QuestionBankPage() {
  const { items, hasMore, nextCursor, setPage, upsert } = useQuestionBankStore();
  ...
  const refresh = async () => {
    const data = await listQuestionBanks({
    ...
    >
      ...
      <div className="relative">
        <PageHeader
          ...
          description="Shuffled quizzes generated from your materials"
          actions={
            <>
              <select
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="relative rounded-[1.35rem] border border-dashed border-[#0c2420]/15 bg-white/70 px-8 py-14 text-center backdrop-blur">
          ...
          <p className="mt-2 text-sm text-[#5a7a73]">
            Generate quizzes from a completed document in your library
          </p>
          <Button
            onClick={() => setCreateOpen(true)}
        ...
        <div className="relative grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {items.map((bank) => (
            <BankCard key={bank.id || bank.document_id} bank={bank} />
          ))}
        </div>
      )}
